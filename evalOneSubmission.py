...
parser = argparse.ArgumentParser(
    usage="python3 evalOneSubmission.py",
    description=r"""
  TADPOLE Evaluation Script:
  The program computes the following metrics:
...
def parseData(d4Df, forecastDf, diagLabels):
    ...
    for s in range(nrSubj):
        ...
        ventriclesEstimLo[s] = currSubjData["Ventricles_ICV 50% CI lower"].iloc[
            indexMinMri
        ]
        ventriclesEstimUp[s] = currSubjData["Ventricles_ICV 50% CI upper"].iloc[
            indexMinMri
        ]
...
def evalOneSub(d4Df, forecastDf):
  ...
  """
    ...
    forecastDf["Forecast Date"] = [
        datetime.strptime(x, "%Y-%m") for x in forecastDf["Forecast Date"]
    ...
    zipTrueLabelAndProbs, hardEstimClass, adasEstim, adasEstimLo, adasEstimUp, ventriclesEstim, ventriclesEstimLo, ventriclesEstimUp, trueDiagFilt, trueADASFilt, trueVentsFilt = parseData(
        d4Df, forecastDf, diagLabels
    )
    ...
    ventsWES = np.sum(ventsCoeffs * np.abs(ventriclesEstim - trueVentsFilt)) / np.sum(
        ventsCoeffs
    )
    ...
    adasCovProb = (
        np.sum((adasEstimLo < trueADASFilt) & (adasEstimUp > trueADASFilt))
        / trueADASFilt.shape[0]
    )
    ...
    ventsCovProb = (
        np.sum(
            (ventriclesEstimLo < trueVentsFilt) & (ventriclesEstimUp > trueVentsFilt)
        )
        / trueVentsFilt.shape[0]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--d4File",
        dest="d4File",
        help="CSV file containing the D4 dataset. "
    ...
    if args.leaderboard:
        if ("TADPOLE_Submission_Leaderboard_" not in forecastFile) or (
            not forecastFile.endswith(".csv")
        ):
            raise ValueError(
                "Leaderboard submission filename is not in the correct format: TADPOLE_Submission_Leaderboard_TeamName.csv"
            )
    else:
        if (not forecastFile) or ("TADPOLE_Submission_" not in forecastFile) or (
            not forecastFile.endswith(".csv")
        ):
            raise ValueError(
    ...
    mAUC, bca, adasMAE, ventsMAE, adasWES, ventsWES, adasCPA, ventsCPA = evalOneSub(
        d4Df, subDf
    )
