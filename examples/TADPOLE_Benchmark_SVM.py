...
Dtadpole_sorted.to_csv(
    Path("IntermediateData/BenchmarkSVMFeaturesTADPOLE.csv"), index=False
)
...
SD2.to_csv(Path("IntermediateData/ToPredict_D2.csv"), index=False)

# SVM for TADPOLE
print("Train SVM for Diagnosis and SVR for ADAS and Ventricles")
# Read Data

str_in = Path("IntermediateData/BenchmarkSVMFeaturesTADPOLE.csv")
...
## Create TestSet
S = pd.read_csv(
    Path("IntermediateData/ToPredict_D2.csv"), header=None
)
S = S.values
...
str_out_final = os.path.join(
    Path("IntermediateData/TADPOLE_Submission_BenchmarkSVM.csv")
)
output.to_csv(str_out_final, header=True, index=False)

#
# print("Evaluate predictions")
# R = pd.read_csv(Path("IntermediateData/D4_dummy.csv"))
#
